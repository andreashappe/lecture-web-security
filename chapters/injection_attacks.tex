...
Zumeist wird dies durch Unterschiede bei der Request-Behandlung zwischen Front-End und Backend-Systemen ermöglicht. Bei einem häufigen Setup empfängt ein Frontend-System die Benutzerrequests aller Browser. Diese werden eventuell gefilteret und dann über eine Verbindung an ein Backend-System weitergeleitet. Hier wird zumeist aus Performance-Gründen nur eine Verbindung verwendet, dass heißt auch, dass Requests unterschiedlicher Browser nacheinander über diese Verbindung übertragen werden. Das Backend-System empfängt nun über diese Verbindung die Anfragen, muss diese nun wieder in einzelne Anfragen zerlegen und schlußendlich abarbeiten. Werden hier die Anfrage-Grenzen (also die Länge der HTTP Requests) unterschiedlich interpretiert, kann dies dazu führen, dass der Angreifer im Backend-System dies dazu ausnutzen kann, Daten ``vor'' einem Folgerequest anzuhängen.

Häufig kann dies durch modifizierte Längen-Header inititiert werden. Beispielsweise könnte das Front-End System den \textit{Content-Length} Header verwenden, während das Backend-System den \textit{Transfer-Encoding} Header zur Längenbestimmung verwendet.
